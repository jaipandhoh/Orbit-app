...
- **`posts.status` column**: Added via `ALTER TABLE posts ADD COLUMN IF NOT EXISTS status VARCHAR(20) DEFAULT 'draft' CHECK (status IN ('draft','scheduled','published'))` in `ensureSchema()`. Only three values for now; Feature 2 will expand the enum.
